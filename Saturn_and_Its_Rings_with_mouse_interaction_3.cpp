...
void drawSmallTexturedObjects(GLfloat time) {
    ...
    int numObjects = 100;    // Number of small objects
...
void drawOuterObjects(GLfloat time, GLfloat innerRadius) {
    ...
    int numOuterObjects = 78; // More objects in outer orbit
    for (int i = 0; i < numOuterObjects; ++i) {
        ...
        drawSphere(0.01f, 10, 10); // Smaller sphere for outer objects
        glPopMatrix();
    }
}
...
void resize(int width, int height) {
    ...
    // Set up the perspective (adjust as needed)
    gluPerspective(45.0, (GLdouble)width / (GLdouble)height, 1.0, 200.0);
