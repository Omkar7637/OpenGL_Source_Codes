...
// Function to draw Saturn's rings with a gradient effect
void drawRings(float innerRadius, float outerRadius, int numSegments) {
    for (int i = 0; i < numSegments; ++i) {
        ...
        // Calculate gradient colors based on the angle
        float colorFactor = (float)i / (float)numSegments;
        glColor3f(0.5f + 0.5f * colorFactor, 0.2f + 0.3f * colorFactor, 0.1f); // Gradient color
...
void display(void) {
    ...
    glColor3f(1.0f, 0.7f, 0.3f); // Color of Saturn
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Detailed Saturn with Rings");
