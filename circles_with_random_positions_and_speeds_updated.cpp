...
GLfloat rotationSpeed = 0.5f; // Default rotation speed
...
void initialize(void) {
    ...
    // Load texture and apply filtering
    ...
    glLightfv(GL_LIGHT0, GL_SPECULAR, light_specular); // Enable specular light
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        ...
        case '-': // Decrease rotation speed
            rotationSpeed = rotationSpeed > 0.1f ? rotationSpeed - 0.1f : 0.0f;
            break;
